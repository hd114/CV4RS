...
def plot_micro_mAP(micro_mAP_data, labels, output_path):
    ...
    plt.fill_between(x_smooth, min_smooth, max_smooth, color='lightblue', alpha=0.5, label="Range (min-max, scenarios)")

    # Plot mean line for the scenarios
    plt.plot(x_smooth, mean_smooth, color='blue', linewidth=2, label="Average micro mAP (scenarios)")
