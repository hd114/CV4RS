...
def apply_pruning(model: torch.nn.Module, pruning_ratio: float = 0.8) -> dict:
    ...
    for name, param in model.state_dict().items():
        if param.requires_grad:
            ...
            # Single concise log per layer
            print(
                f"Layer: {name} | Mask shape: {mask.shape} | Pruned: {num_pruned}/{num_weights} ({pruning_ratio:.2f})")
        else:
            # Keep non-trainable parameters (e.g., running_mean, running_var) unchanged
