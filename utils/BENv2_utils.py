...
_prithvi_bandnames = ["B02", "B03", "B04", "B05", "B06", "B07"]
    ...
    def _interpolate(img_data):
        if not img_data.shape[-2:] == (img_size, img_size):
            return F.interpolate(
                ...
                mode=upsample_mode,
                align_corners=(
                    True if upsample_mode in ["bilinear", "bicubic"] else None
                ),
            ).squeeze()
        else:
...
def ben_19_labels_to_multi_hot_np(
        labels: Iterable[str], lex_sorted: bool = True
) -> np.array:
    """
    ...
    if isinstance(bands, str):
        if bands in _s1_bandnames or bands in _s2_bandnames:
            ...
            assert bands in STANDARD_BANDS.keys(), (
                "Band combination unknown, please use a list of strings or one of "
                f"{STANDARD_BANDS.keys()}"
            )
...
class BENv2LDMBReader:
    ...
    def __getitem__(self, key: str):
        ...
        if self.uses_s1:
            ...
            ), "S1 bands are used, but no mapping is provided"
